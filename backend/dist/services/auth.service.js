...
class AuthService {
    ...
    static async signup(data) {
        ...
        if (existingUser) {
            throw ApiError_1.default.conflict('User with this email already exists');
        }
        // Validate UPI PIN format
        if (!data.upiPin || !/^\d{4,6}$/.test(data.upiPin)) {
            throw ApiError_1.default.badRequest('UPI PIN must be 4-6 digits');
        }
        // Hash password
        const hashedPassword = await bcrypt_1.default.hash(data.password, env_1.env.BCRYPT_ROUNDS);
        // Hash UPI PIN
        const hashedUpiPin = await bcrypt_1.default.hash(data.upiPin, env_1.env.BCRYPT_ROUNDS);
    ...
    static async login(data) {
        ...
        if (!user) {
            throw ApiError_1.default.unauthorized('Invalid credentials');
        }
        // Verify password
    ...
    static async refreshAccessToken(refreshToken) {
        try {
            ...
            throw ApiError_1.default.unauthorized('Invalid refresh token');
        }
    }
    /**
     * Verify UPI PIN
     ...
     */
    static async verifyUpiPin(userId, pin) {
        // DEBUG
        ...
        if (!pin || pin.length < 4 || pin.length > 6) {
            ...
            throw ApiError_1.default.badRequest('Invalid PIN. PIN must be 4-6 digits.');
        }
        const user = await database_1.prisma.user.findUnique({
            where: { id: userId },
            select: {
        ...
        if (!user.upiPin) {
            throw ApiError_1.default.badRequest('UPI PIN not set. Please contact support.');
        }
        ...
        if (user.pinLockedUntil && new Date() < user.pinLockedUntil) {
            ...
            throw ApiError_1.default.forbidden(`Account locked due to multiple wrong PIN attempts. Try again in ${minutesLeft} minute${minutesLeft > 1 ? 's' : ''}.`);
        }
        // Verify PIN
        const isPinValid = await bcrypt_1.default.compare(pin, user.upiPin);
        if (!isPinValid) {
            ...
            if (newAttempts >= 3) {
                ...
                throw ApiError_1.default.forbidden('Account locked for 15 minutes due to 3 wrong PIN attempts.');
            }
            else {
                ...
                throw ApiError_1.default.unauthorized(`Wrong UPI PIN. ${3 - newAttempts} attempt${3 - newAttempts > 1 ? 's' : ''} remaining.`);
            }
        }
        ...
        logger_1.logger.info(`UPI PIN verified successfully for user: ${userId}`);
        return true;
    }
    /**
     * Get user by ID
